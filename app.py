...
.app-header h1 { color: #f0f6ff; font-size: 2rem; font-weight: 700; margin: 0; }
...
.card h3 { color: #e2e8f0; font-size: 0.95rem; font-weight: 600; margin: 0 0 1rem 0; }
...
.conf-bar-bg { background: #1a2540; border-radius: 999px; height: 8px; margin: 4px 0 10px 0; overflow: hidden; }
...
.rapport-title { color: #60a5fa; font-size: 1rem; font-weight: 700; margin-bottom: 1rem; }
.rapport-row { display: flex; justify-content: space-between; padding: 0.4rem 0;
    border-bottom: 1px solid #1e3a5f; font-size: 0.83rem; }
.rapport-label { color: #4a6080; }
.rapport-value { color: #e2e8f0; font-weight: 600; }

.gcam-label { font-size: 0.75rem; color: #2d4a6a; text-align: center; margin-top: 0.3rem; }

section[data-testid="stSidebar"] { background: #080c14; border-right: 1px solid #1e3a5f; }
section[data-testid="stSidebar"] .stMarkdown p { color: #7fa3c8; font-size: 0.83rem; }

[data-testid="stFileUploader"] {
    background: #0f1623; border: 2px dashed #1e3a5f; border-radius: 14px; padding: 0.8rem;
}
</style>
...
with st.sidebar:
    ...
    st.markdown("**⚙️ Modèle :** MobileNetV2 Fine Tuning")
    st.markdown("**🎯 Accuracy :** 96.21%")
    st.markdown("---")
    st.markdown("**🏷 Classes :**")
    for cls, info in CLASS_INFO.items():
        st.markdown(
            f"{info['emoji']} **{info['label']}**  \n"
            f"<small style='color:#2d4a6a'>{info['desc']}</small>",
            unsafe_allow_html=True
        )
    st.markdown("---")
    st.markdown(
        "<div style='font-size:0.78rem;color:#2d4a6a;text-align:center'>"
        "⭐ MobileNetV2 Fine Tuning<br>"
...
with col_l:
    ...
    if uploaded:
        ...
        st.markdown("""
        <div style="text-align:center;padding:2.5rem 1rem;color:#1e3a5f">
            <div style="font-size:3rem">🩻</div>
            <div style="font-size:0.88rem;margin-top:0.5rem;color:#2d4a6a">Aucune image sélectionnée</div>
            <div style="font-size:0.75rem;color:#1e3a5f">Formats : JPG, PNG</div>
        </div>
        """, unsafe_allow_html=True)
...
with col_r:
    ...
    if uploaded:
        ...
        if model:
            ...
            <p style='color:#4a6080;font-size:0.82rem;text-align:center'>{info['desc']}</p>
            """, unsafe_allow_html=True)

            # Severity + Confidence
            if top_conf >= 90: lv, lc = "Très élevée ✅", "#22c55e"
            elif top_conf >= 70: lv, lc = "Élevée 🟡", "#f59e0b"
            else: lv, lc = "Faible ⚠️", "#ef4444"
            ...
            <div style="display:flex;gap:0.6rem;justify-content:center;margin-bottom:1rem">
                <div style="background:#0a1628;border:1px solid #1e3a5f;border-radius:8px;
                            padding:0.4rem 0.9rem;font-size:0.78rem;color:#4a6080">
            ...
            for i, cls in enumerate(CLASSES):
                ...
                ls = "font-weight:700;color:#e2e8f0;" if is_top else "color:#2d4a6a;"
                st.markdown(f"""
            ...
            # Grad-CAM
            st.markdown("---")
            st.markdown("**🌡️ Grad-CAM — Zone activée :**")
            overlay = gradcam(model, arr, pred_idx)
            if overlay:
                gc1, gc2 = st.columns(2)
                with gc1: st.image(img.resize((224,224)), caption="IRM originale", use_column_width=True)
                with gc2: st.image(overlay, caption="🌡️ Grad-CAM", use_column_width=True)
        ...
        st.markdown("""
        <div style="text-align:center;padding:5rem 1rem;color:#1e3a5f">
